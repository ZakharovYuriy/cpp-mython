...
ObjectHolder VariableValue::Execute(Closure& closure, Context&) {
    ...
    ObjectHolder result;
    for (const auto& id : dotted_ids_) {//; counter < dotted_ids_.size(); ++counter      
...
void Print::ExecutePrint(size_t pos, runtime::Closure& closure, runtime::Context& context) {
...
ObjectHolder MethodCall::Execute(Closure& closure, Context& context) {
    ...
    for (size_t i = 0; i < args_.size(); ++i) {
        args_converted.push_back(move(args_[i].get()->Execute(closure, context)));
    }
    return object_.get()->Execute(closure, context).TryAs<runtime::ClassInstance>()->Call(method_, args_converted, context);
...
ObjectHolder Stringify::Execute(Closure& closure, Context& context) {   
    ...
    if (argument_.get()->Execute(closure, context)) {
        argument_.get()->Execute(closure, context)->Print(string_stream, context);
    }
    else {
...
ClassDefinition::ClassDefinition(ObjectHolder cls):cls_(cls){//cls_(*cls.TryAs<runtime::Class>()) {
}

ObjectHolder ClassDefinition::Execute(Closure& closure, Context&) {
    closure[cls_.TryAs<runtime::Class>()->GetName()] = ObjectHolder::Own(runtime::ClassInstance(*cls_.TryAs<runtime::Class>()));
    return ObjectHolder();
}
...
ObjectHolder FieldAssignment::Execute(Closure& closure, Context& context) { 
    ...
    result = result.TryAs<runtime::ClassInstance>()->Fields()[field_name_] = rv_->Execute(closure, context);
    return result;
}
...
ObjectHolder Comparison::Execute(Closure& closure, Context& context) {
    if (comparator_(lhs_.get()->Execute(closure, context), rhs_.get()->Execute(closure, context), context))
    {
        return  ObjectHolder::Own(runtime::Bool(true));
    }
    else {
...
NewInstance::NewInstance(const runtime::Class& class_, std::vector<std::unique_ptr<Statement>> args): new_class_(class_), args_(move(args)) {
}

NewInstance::NewInstance(const runtime::Class& class_) :/*clas_ptr(std::make_shared<runtime::Class>(class_)),*/ new_class_(class_) {
}
