...
void TestNumbers() {
	...
	// Negative numbers are formed at the parsing stage
	ASSERT_EQUAL(lexer.NextToken(), Token(token_type::Char { '-' }));
...
void TestStrings() {
	...
	ASSERT_EQUAL(lexer.NextToken(), Token(token_type::String {
			"long string with a double quote \" inside"s }));
	ASSERT_EQUAL(lexer.NextToken(), Token(token_type::String {
			"another long string with single quote ' inside"s }));
}
...
x = 1
  ...
  z = 3
	...
	ASSERT_EQUAL(lexer.NextToken(), Token(token_type::Newline { }));
	// An empty string consisting only of whitespace characters does not change the current indentation,
	// therefore, the next token is Id, not Dedent
